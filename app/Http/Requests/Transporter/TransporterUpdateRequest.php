...
class TransporterUpdateRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', 'unique:transporters,name,' . $this->route('id') .',id', 'max:255'],
            'document_number' => ['required', 'string', 'min:5', 'max:20', 'unique:transporters,document_number,' . $this->route('id') .',id'],
            'telephone_number' => ['required', 'string', 'size:10'],
            'email' => ['required', 'string', 'email', 'unique:transporters,email,' . $this->route('id') .',id', 'max:255']
        ];
    }
