...
class OrderReturnDetailUpdateRequest extends FormRequest
{
    ...
    protected function prepareForValidation()
    {
        ...
        $order_return_detail = OrderReturnDetail::with('order_return_detail_quantities.order_detail_quantity')->findOrFail($this->route('id'));
        ...
        foreach($order_return_detail_quantities as $order_return_detail_quantity) {
            ...
            $order_return_detail_quantity['max'] = $order_detail_quantity ? $order_detail_quantity->quantity - $order_return_details->flatten()->where('order_detail_quantity.size_id', $order_return_detail_quantity['size_id'])->pluck('quantity')->sum() + ($order_return_detail->order_detail_id == $order_detail->id ? $order_return_detail->order_return_detail_quantities->where('order_detail_quantity.size_id', $order_return_detail_quantity['size_id'])->pluck('quantity')->sum() : 0) : 0;
    ...
    public function rules()
    {
        $rules = [
            ...
            'order_detail_id' => ['required', 'exists:order_details,id', 'unique:order_return_details,order_detail_id,' . $this->route('id') . ',id,order_return_id,' . $this->input('order_return_id')],
