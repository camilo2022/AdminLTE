...
class SupplyTypeStoreRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', 'unique:supply_types,name', 'max:255'],
            'code' => ['required', 'string', 'unique:supply_types,code', 'max:255'],
            'is_cloth' => ['required', 'boolean', Rule::when($this->input('is_cloth'), 'unique:supply_types,is_cloth')]
        ];
    }
