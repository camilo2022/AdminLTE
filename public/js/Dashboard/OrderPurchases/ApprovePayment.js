function ApprovePaymentOrderSeller(id, status = true) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: `/Dashboard/Orders/Seller/ApprovePayment`,
                type: 'PUT',
                data: {
...
function ApprovePaymentOrderSellerAjaxSuccess(response) {
...
function ApprovePaymentOrderSellerAjaxError(xhr) {
    if(xhr.status === 403) {
