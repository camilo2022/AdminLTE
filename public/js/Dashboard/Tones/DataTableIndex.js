let tableTones = $('#tones').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
        url: `/Dashboard/Tones/Index/Query`,
        type: 'POST',
    ...
    columns: [
        ...
        },
        {
            ...
            render: function (data, type, row) {
                ...
                if (data === null) {
                    btn += `<a onclick="EditToneModal(${row.id})" type="button"
                    class="btn btn-primary btn-sm mr-2" title="Editar tono">
                        <i class="fas fa-pen text-white"></i>
                    </a>`;

                    btn += `<a onclick="DeleteTone(${row.id})" type="button"
                    class="btn btn-danger btn-sm mr-2" title="Eliminar tono">
                        <i class="fas fa-trash text-white"></i>
                    </a>`;
                } else {
                    btn += `<a onclick="RestoreTone(${row.id})" type="button"
                    class="btn btn-info btn-sm mr-2" title="Restaurar tono">
