let tableClientTypes = $('#clientTypes').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
        url: `/Dashboard/ClientTypes/Index/Query`,
        type: 'POST',
        ...
        dataSrc: function (response) {
            ...
            return response.data.clientTypes;
        },
    ...
    columns: [
        ...
        },
        {
            ...
            render: function (data, type, row) {
                ...
                if (data === null) {
                    btn += `<a onclick="EditClientTypeModal(${row.id})" type="button"
                    class="btn btn-primary btn-sm mr-2" title="Editar tipo de cliente">
                        <i class="fas fa-pen text-white"></i>
                    </a>`;

                    btn += `<a onclick="DeleteClientType(${row.id})" type="button"
                    class="btn btn-danger btn-sm mr-2" title="Eliminar tipo de cliente">
                        <i class="fas fa-trash text-white"></i>
                    </a>`;
                } else {
                    btn += `<a onclick="RestoreClientType(${row.id})" type="button"
                    class="btn btn-info btn-sm mr-2" title="Restaurar tipo de cliente">
