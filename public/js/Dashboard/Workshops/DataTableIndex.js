let tableWorkshops = $('#workshops').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
        url: `/Dashboard/Workshops/Index/Query`,
        type: 'POST',
    ...
    columns: [
        ...
        },
        {
            ...
            render: function (data, type, row) {
                ...
                if (data === null) {
                    btn += `<a onclick="ShowWorkshopModal(${row.id})" type="button"
                    class="btn btn-info btn-sm mr-2" title="Asignar y remover procesos a taller.">
                        <i class="fas fa-eye text-white"></i>
                    </a>`;

                    btn += `<a onclick="EditWorkshopModal(${row.id})" type="button"
                    class="btn btn-primary btn-sm mr-2" title="Editar taller.">
                        <i class="fas fa-pen text-white"></i>
                    </a>`;

                    btn += `<a onclick="DeleteWorkshop(${row.id})" type="button"
                    class="btn btn-danger btn-sm mr-2" title="Eliminar taller.">
                        <i class="fas fa-trash text-white"></i>
                    </a>`;
                } else {
                    btn += `<a onclick="RestoreWorkshop(${row.id})" type="button"
                    class="btn btn-info btn-sm mr-2" title="Restaurar taller.">
