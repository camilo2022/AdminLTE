function PendingOrderWallet(id, status = true) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: `/Dashboard/Orders/Wallet/Pending`,
                type: 'PUT',
                data: {
...
function PendingOrderWalletAjaxSuccess(response) {
...
function PendingOrderWalletAjaxError(xhr) {
    if(xhr.status === 403) {
