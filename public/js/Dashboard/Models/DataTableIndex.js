let tableModels = $('#models').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
        url: `/Dashboard/Models/Index/Query`,
        type: 'POST',
        ...
        },
        {
            ...
            render: function (data, type, row) {
                ...
                if (data === null) {
                    ...
                    btn += `<a onclick="DeleteModel(${row.id})" type="button"
                    class="btn btn-danger btn-sm mr-2" title="Eliminar modelo de prodcuto">
                        <i class="fas fa-trash text-white"></i>
                    </a>`;
                } else {
                    btn += `<a onclick="RestoreModel(${row.id})" type="button"
                    class="btn btn-info btn-sm mr-2" title="Restaurar modelo de prodcuto">
