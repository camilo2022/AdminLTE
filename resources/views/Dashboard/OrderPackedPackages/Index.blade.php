                    ...
                    <div class="card">
                        <div class="card-header p-2">
                            <ul class="nav nav-pills">
                                <li class="nav-item">
                                    <a class="btn btn-info text-white" id="IndexOrderPackedDetail" data-id="{{ $orderPacked->id }}" onclick="IndexOrderPackedDetail({{ $orderPacked->id }})" type="button" title="Orden de alistamiento y empacado del pedido.">
                                        ORDEN DE DESPACHO: {{ $orderPacked->order_dispatch->consecutive }}
                                    </a>
                                </li>
                                <li class="nav-item ml-auto">
                                    <a class="btn btn-primary" type="button" onclick="StoreOrderPackedPackage({{ $orderPacked->id }}, {{ json_encode($packageTypes) }})" title="Nuevo empaque para alistar y empacar la orden de despacho.">
...
@endsection
@section('script')
<script src="{{ asset('js/Dashboard/OrderPackedPackages/Store.js') }}"></script>
